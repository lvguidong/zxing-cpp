...
int read_image(Ref<LuminanceSource> source, bool hybrid, string expected, char* databuff) {
  ...
  if (test_mode && results.size() == 1) {
    ...
    //       << "  Detected: " << result << endl;
      memcpy(databuff,result.c_str(), strlen(result.c_str()));
      res = -6;
    } else {
...
int get_qrcode(const char* name, char*databuff)
{
   string filename = name;
    ...
    int hresult = read_image(source, false, expected, databuff);
    return hresult;
}
...
int main1(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    int hresult = 1;
    if (use_hybrid) {
      hresult = read_image(source, true, expected, NULL);
    }
    if (use_global && (verbose || hresult != 0)) {
      gresult = read_image(source, false, expected,NULL);
